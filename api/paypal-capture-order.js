// Vercel Serverless — capture PayPal order + issue signed analysis token
...
export default async function handler(req, res) {
  ...
  const { orderID, amount, sessionId } = req.body || {};
  ...
  try {
    ...
    if (capture.status !== 'COMPLETED') {
      return res.status(402).json({
        error: `Zahlung nicht abgeschlossen (Status: ${capture.status || 'unbekannt'})`,
      });
    }

    // Payment confirmed server-side — now issue a signed analysis token
    ...
    return res.status(200).json({ token, paypalOrderId: capture.id });
  } catch (err) {
